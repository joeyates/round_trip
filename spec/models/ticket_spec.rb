...
module RoundTrip
  describe Ticket do
    ...
    context 'create_from_* methods' do
      before do
        Ticket.stub(:create!).with(anything)
      end
      ...
      describe '.create_from_redmine_resource' do
        ...
        let(:issue_resource) { double('Redmine::Issue', resource_attribs) }
        let(:issue_resource_with_trello_id) { double('Redmine::Issue', resource_attribs_with_trello_id) }
      ...
      describe '.create_from_trello_card' do
        ...
        let(:trello_card) { double('Trello::Card', card_attributes) }
        ...
        it 'extracts redmine ids' do
          trello_card = double('Trello::Card', card_attributes.merge(:description => card_description_with_redmine_id))
    ...
    describe '.check_repeated_redmine_ids' do
      let(:trello_has_redmine_id_relation) { double('ActiveRecord::Relation') }

      before do
        Ticket.stub(:trello_has_redmine_id).and_return(trello_has_redmine_id_relation)
      end

      it 'raises an error for duplicate redmine ids' do
        result = {'123454f947c12c3479004705' => 2, '123454f947c12c3479004706' => 3}
        trello_has_redmine_id_relation.stub(:for_redmine_project => double(:group => double(:having => double(:count => result))))
    ...
    describe '.check_repeated_trello_ids' do
      let(:redmine_has_trello_id_relation) { double('ActiveRecord::Relation') }

      before do
        Ticket.stub(:redmine_has_trello_id).and_return(redmine_has_trello_id_relation)
      end

      it 'raises an error for duplicate trello ids' do
        result = {87654 => 2}
        redmine_has_trello_id_relation.stub(:for_trello_board => double(:group => double(:having => double(:count => result))))
    ...
    describe '.check_repeated_redmine_subjects' do
      let(:for_redmine_project_relation) { double('ActiveRecord::Relation') }
      let(:redmine_subject_1) { 'Subject 1' }

      before do
        Ticket.stub(:for_redmine_project).and_return(for_redmine_project_relation)
      end

      it 'raises an error for repeated redmine issue subjects' do
        result = {redmine_subject_1 => 2}
        for_redmine_project_relation.stub(:group => double(:having => double(:count => result)))
    ...
    describe '.check_repeated_trello_names' do
      let(:for_trello_board_relation) { double('ActiveRecord::Relation') }
      let(:trello_name_1) { 'Subject 1' }

      before do
        Ticket.stub(:for_trello_board).and_return(for_trello_board_relation)
      end

      it 'raises an error for repeated trello card names' do
        result = {trello_name_1 => 2}
        for_trello_board_relation.stub(:group => double(:having => double(:count => result)))
    ...
    describe '#merge_redmine' do
      before do
        redmine_ticket.stub(:destroy)
        trello_ticket.stub(:save!)
